...
public class NetcodeSynchronizer : NetworkBehaviour
{
    ...
    {
        SceneName = name;
        AssetManager.AssetManager.LoadScene(name, (prefabName, position, rotation) => SpawnPrefab(prefabName, position, false));
        SceneLoaded = true;
    }
    ...
    GameObject SpawnPrefab(string prefabName, Vector3 position, uint networkId, bool broadcastSpawn)
    {
        ...
        for (int i = 0; i < NetworkPrefabs.Count; i++)
        {
            ...
            {
                if (ObjectGroups.Game != null)
                { newObject = GameObject.Instantiate(NetworkPrefabs[i], position, Quaternion.identity, ObjectGroups.Game); }
                else
        ...
        if (newObject == null)
        {
            newObject = AssetManager.AssetManager.InstantiatePrefab(prefabName, false);
            if (newObject != null) newObject.transform.position = position;
        }
    ...
    public void DestroyObject(uint networkId)
    {
        if (!isActiveAndEnabled) return;
        if (!NetworkManager.IsServer)
        { throw new NetcodeErrors.PermissionException($"Client can not destroy object"); }
