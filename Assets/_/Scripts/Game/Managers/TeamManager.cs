...
namespace Game.Managers
{
    public class TeamManager : SingleInstance<TeamManager>
    {
        ...
        readonly List<string> TeamIDs = new();

        [SerializeField] List<Team> teams = new();
        [SerializeField] List<FuckYou> fuckYous = new();

        [SerializeField, Button(nameof(DoAutoFriends), true, false, "Auto-Friends")] string btnAutoFriends;

        void DoAutoFriends()
        {
            foreach (Team team in teams)
            { SetFuckYou(team.ID, team.ID, -1f); }
        }
        ...
        internal void SetFuckYou(string teamA, string teamB, float value)
        {
            ...
            fuckYous.Add(new FuckYou()
            {
                FuckYouValue = value,
                Team1 = teamA,
                Team2 = teamB,
                Team1Hash = GetTeam(teamA).Hash,
                Team2Hash = GetTeam(teamB).Hash,
            });
        }
    }
}
