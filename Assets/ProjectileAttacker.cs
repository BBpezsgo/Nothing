...
internal class ProjectileAttacker : AttackerBase, IHaveAssetFields
{
    ...
    bool ThinkOnTarget(Projectile projectile)
    {
        ...
        float? angle_ = Utilities.Ballistics.AngleOfReach2(v, turret.ShootPosition, projPosition);

        float? t = angle_.HasValue ? Utilities.Ballistics.TimeToReachDistance(v, angle_.Value, d) : null;
