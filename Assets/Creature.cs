...
public class Creature : NetworkBehaviour, IDamagable
{
    ...
    [SerializeField, ReadOnly] protected Terrain Terrain;
    ...
    protected virtual Vector3 FindNewDestination()
    {
        ...
        result.x += Random.Range(-20f, 20f);
        result.z += Random.Range(-20f, 20f);

        result.z = Terrain.SampleHeight(result) + Terrain.transform.position.y;
    ...
    protected virtual Vector3 FindFleeDestination()
    {
        ...
        result.z += Random.Range(-5f, 5f);
        result += transform.forward * 20f;
